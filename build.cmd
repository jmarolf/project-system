...
powershell -ExecutionPolicy ByPass -Command "& """%Root%eng\common\build.ps1""" -configuration %BuildConfiguration% -restore -pack:%OptPack% -build:%OptBuild% -rebuild:%OptRebuild% -deploy:%OptDeploy% -test:%OptTest% -integrationTest:%OptIntegrationTest% -binaryLog:%OptLog% %PropRootSuffix%"
exit /b %ERRORLEVEL%
